...
def send_command(shared):
    ...
    if cur_state == 0:
        ...
        cur_account += 1
        cur_account_name = str(cur_account).zfill(NUM_PREFIX_BLOCK * DES_BLOCK_SIZE - 6 - cur_amount_len - 1)
...
def mitm(buff, direction, shared):
    ...
    if direction == CLIENT2SERVER:
        if cur_state == 0:
            ...
            cur_state += 1
            cur_buffer = buff[:NUM_PREFIX_BLOCK * DES_BLOCK_SIZE]
            send_command(shared)
        elif cur_state == 2:
            if buff[:NUM_PREFIX_BLOCK * DES_BLOCK_SIZE] == cur_buffer:
